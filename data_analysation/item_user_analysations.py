...
def correlations_between_items_a_user_order(orders, items):
    """find correlations between features/brand from all items one user buy
    ...
    # mapped_order_item_features_category.to_csv('mapped_order_item_features_category.csv', sep='|')
...
def shared_features_per_user(orders_items_features):
    ...
    brand_count_per_user = orders_items_features.groupby(['userID', 'brand']).sum().reset_index().sort_values(['userID', 'order'])[['userID', 'brand', 'order']].reset_index(drop=True)
    feature_1_count_per_user = orders_items_features.groupby(['userID', 'feature_1']).sum().reset_index().sort_values(['userID', 'order'])[['userID', 'feature_1', 'order']].reset_index(drop=True)
    feature_2_count_per_user = orders_items_features.groupby(['userID', 'feature_2']).sum().reset_index().sort_values(['userID', 'order'])[['userID', 'feature_2', 'order']].reset_index(drop=True)
    feature_3_count_per_user = orders_items_features.groupby(['userID', 'feature_3']).sum().reset_index().sort_values(['userID', 'order'])[['userID', 'feature_3', 'order']].reset_index(drop=True)
    feature_4_count_per_user = orders_items_features.groupby(['userID', 'feature_4']).sum().reset_index().sort_values(['userID', 'order'])[['userID', 'feature_4', 'order']].reset_index(drop=True)
    feature_5_count_per_user = orders_items_features.groupby(['userID', 'feature_5']).sum().reset_index().sort_values(['userID', 'order'])[['userID', 'feature_5', 'order']].reset_index(drop=True)
    ...
    brand_count_per_user['share_brand'] = brand_count_per_user.apply(lambda row: row['order'] / items_ordered_per_user['order'][row['userID']], axis=1)
    feature_1_count_per_user['share_feature_1'] = feature_1_count_per_user.apply(lambda row: row['order'] / items_ordered_per_user['order'][row['userID']], axis=1)
    feature_2_count_per_user['share_feature_2'] = feature_2_count_per_user.apply(lambda row: row['order'] / items_ordered_per_user['order'][row['userID']], axis=1)
    feature_3_count_per_user['share_feature_3'] = feature_3_count_per_user.apply(lambda row: row['order'] / items_ordered_per_user['order'][row['userID']], axis=1)
    feature_4_count_per_user['share_feature_4'] = feature_4_count_per_user.apply(lambda row: row['order'] / items_ordered_per_user['order'][row['userID']], axis=1)
    feature_5_count_per_user['share_feature_5'] = feature_5_count_per_user.apply(lambda row: row['order'] / items_ordered_per_user['order'][row['userID']], axis=1)
    pass
...
def distribution_of_dominant_features_per_user(orders_items_features):
    ...
    orders_items_features['order'] = orders_items_features.apply(lambda row: 1, axis=1)

    # number_of_different_items_with_same_brand_a_user_order
    same_brand_different_items = orders_items_features.groupby(['userID', 'brand']).sum().reset_index()[['userID', 'brand', 'order']]
    same_feature_1_different_items = orders_items_features.groupby(['userID', 'feature_1']).sum().reset_index()[['userID', 'feature_1', 'order']]
    same_feature_2_different_items = orders_items_features.groupby(['userID', 'feature_2']).sum().reset_index()[['userID', 'feature_2', 'order']]
    same_feature_3_different_items = orders_items_features.groupby(['userID', 'feature_3']).sum().reset_index()[['userID', 'feature_3', 'order']]
    same_feature_4_different_items = orders_items_features.groupby(['userID', 'feature_4']).sum().reset_index()[['userID', 'feature_4', 'order']]
    same_feature_5_different_items = orders_items_features.groupby(['userID', 'feature_5']).sum().reset_index()[['userID', 'feature_5', 'order']]

    # number_of_different_items_a_user_order
    different_items_per_user = orders_items_features.groupby(['userID']).sum().reset_index()[['userID', 'order']].set_index(['userID'])

    # calculate distribution of brand-/feature-values of the user
    same_brand_different_items['distribution_brand'] = same_brand_different_items.apply(lambda row: row['order'] / different_items_per_user['order'][row['userID']], axis=1)
    same_feature_1_different_items['distribution_feature_1'] = same_feature_1_different_items.apply(lambda row: row['order'] / different_items_per_user['order'][row['userID']], axis=1)
    same_feature_2_different_items['distribution_feature_2'] = same_feature_2_different_items.apply(lambda row: row['order'] / different_items_per_user['order'][row['userID']], axis=1)
    same_feature_3_different_items['distribution_feature_3'] = same_feature_3_different_items.apply(lambda row: row['order'] / different_items_per_user['order'][row['userID']], axis=1)
    same_feature_4_different_items['distribution_feature_4'] = same_feature_4_different_items.apply(lambda row: row['order'] / different_items_per_user['order'][row['userID']], axis=1)
    same_feature_5_different_items['distribution_feature_5'] = same_feature_5_different_items.apply(lambda row: row['order'] / different_items_per_user['order'][row['userID']], axis=1)

    pass


def mean_number_of_item_at_a_time(orders):
    """how many items gets ordered at one time from same user, mean
    ...
    items_at_once['k'] = items_at_once.apply(lambda row: 1 if (row['min'] + row['max']) / 2 >= row['mean'] else 0, axis=1)

    items_at_once = items_at_once.sort_values(['sum'], ascending=False).reset_index()[['itemID', 'mean', 'max', 'min', 'sum', 'k']]
...
if __name__ == '__main__':
    ...
    mean_number_of_item_at_a_time(orders)
    # shared_features_per_user(order_brand_features)
    # distribution_of_dominant_features_per_user(order_brand_features)
